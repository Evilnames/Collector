...
### Political Systems
- **politics.py** — Live political layer over towns/guilds/orders. State: `HOUSE_STATES` (per-region ruling house — power/prestige/scandal/posture/treasury), `RELATIONS_SCORE` (-100..+100 between houses, mirrored to `Region.relations` strings), `RELATION_REASONS` (last-8 incident log per pair, for tooltips), `PLAYER_INFLUENCE` (per-region tally + log), `SUCCESSION` (active crises with named contenders + traits), `PLAYER_PLEDGES`, `HOUSE_HEIRS` (yearly-rotating ruler+heir slates from biome-flavored name pools), `ENVOY_REPORTS`. `AGENDA_PROFILES` drives daily drift (martial gains power, pious heals scandal, mercantile grows treasury, etc.). `tick_politics(world, player)` runs daily after dynasty_events: recomputes power, applies agenda drift, checks bankruptcy, drains sabotage + dynasty hooks, resolves succession crises (~30 days), decays scandals. Eleven player actions: bribe_official, sponsor_heir, broker_marriage, fund_rivalry, leak_scandal, pledge_order, spread_rumor, endow_monastery, arrange_tournament, send_envoy, demand_hostage. `power_breakdown(rid)` decomposes the score for the deep panel. Newswire entries piggyback on `industry_events.NEWSWIRE`.
- **UI/court.py** — `CourtMixin` grafted onto reputation_screen. Tabs: Standing / Houses / Web / Newswire / Orders / Actions. Houses tab: grid of cards (sigil, power/prestige/scandal bars, posture, influence tier); clicking a card opens the **House Deep Panel** (ruler+heir, full power breakdown, backing guilds, sworn orders, succession crisis with contender support, envoy report, influence log, recent newswire mentions). Web tab: force-directed graph with node size scaling with power, agenda-tinted backing disc, scandal halo, yellow pulse on houses mentioned in recent newswire, blue dots for backing guilds; clickable edges show numeric score + last 3 incidents. Newswire / Orders / Actions tabs unchanged structurally.
